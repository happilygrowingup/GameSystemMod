...
public class EvilEternalHunter extends GamePlayer {
    ...
    public static void onTick(TickEvent event) {
        for (int k = 0; k < Game.instances.size(); k++) {
            ...
            if (game.isStarted) {
                if (game.evil instanceof EvilEternalHunter evil) {
                    if (evil.lastPreyChange + 120000 <= new Date().getTime()) {  //Every 120s
                        ...
                        evil.prey = game.humans.get(new Random().nextInt(game.humans.size()));
                    ...
                    if (!game.players.contains(evil.prey)) {
                    ...
                    for (int i = 0; i < evil.trapPos.size(); i++) {
                        ...
                        for (int j = 0; j < game.humans.size(); j++) {
                            GamePlayer human = game.humans.get(j);
